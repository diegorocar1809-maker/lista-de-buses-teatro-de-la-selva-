...
// Cada viaje ahora tiene: buses (3, con capacidad y estado lleno) y pasajeros (con bus asignado)
...
// También migra datos guardados en formatos anteriores (sin buses) al nuevo formato.
function cargarDatos(){
    let d;
    try{
        const guardado = localStorage.getItem(STORAGE_KEY);
        d = guardado ? JSON.parse(guardado) : JSON.parse(JSON.stringify(datosBase));
    }catch(e){
    ...
    Object.keys(d).forEach(v=>{
        // Formato más antiguo: array plano de pasajeros, sin capacidad ni buses
        if(Array.isArray(d[v])){
            d[v] = {buses:busesDefault(),pasajeros:d[v]};
        }

        // Formato anterior (un solo bus con capacidad): {capacidad, pasajeros}
        ...
        // Asegurar que exista el arreglo de buses
        if(!d[v].buses) d[v].buses=busesDefault();

        // Asegurar que cada pasajero tenga el campo "bus"
...
const viaje=document.getElementById('viaje'),
      ...
      busesEl=document.getElementById('buses');
...
<div class="bus-chip ${b.lleno?'lleno':''} ${sobre?'sobre':''}">
    ...
    <div class="bus-actions">
        ...
        <button onclick="toggleLleno(${i})" title="Marcar lleno/libre">
            ${b.lleno?'🔴 Lleno':'🟢 Libre'}
        </button>
    </div>
</div>`;
    ...
    arr
    .filter(p=>p.name.toLowerCase().includes(buscar.value.toLowerCase()))
    .forEach(p=>{
        let idx=pasajeros.indexOf(p);

        let opciones=buses.map((b,i)=>
            `<option value="${i}" ${p.bus===i?'selected':''}>
                ${b.nombre}${b.lleno?' (lleno)':''}
            </option>`
        ).join('');
...
<div class="card ${p.ok?'ok':''}">
    ...
    <div class="bus-row">
        🚌 <select class="bus-select"
            onchange="asignarBus(${idx}, this.value)">
...
// Botón opcional para resetear todo a los datos originales
